...
def waterfall_plot(x, y, bottom, color, bar_text, fname=None):
    ...
    ax.set_title('Comparison of different cost premiums between \nimported and domestically manufactured components')

    h[3].set_linestyle('--')
    h[3].set_linewidth(1.75)
    h[3].set_edgecolor('k')

    ax.text(x[1], 2000, bar_text['transit'], horizontalalignment='center',)
    ax.text(x[2], 2000, bar_text['factory'], horizontalalignment='center',)
    ax.text(x[3], 2000, bar_text['margin'], horizontalalignment='center',)
...
def area_time_plot(x, y, color, fname=None):
    """Area plot showing changin component cost over time"""
    ...
    # Define margin
    ax.fill_between(x, y1 / y_init, np.ones(len(x)), color=color['Cost margin'], label='Margin')

    final_margin = round( 100* (1 - y1[-1] / y_init), 1)

    y_margin = ((1 + y1[-1] / y_init) /2)

    margin_text = '   ' + str(final_margin) + '% CapEx margin relative to \n   European imports can cover \n   local differences in wages, \n   taxes, financing, etc'
    ...
    ax.set_ylabel('CapEx breakdown relative to \ncomponents imported from Europe, %')
